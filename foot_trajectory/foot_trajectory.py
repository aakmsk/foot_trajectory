...
# class FootTrajectoryGeneratorBase:
#     def __init__(self, base_freq, phi_init):
#         self.base_freq = base_freq
#         self.phi_init = phi_init

#     def compute_phi(self, t, f_offset):
#         return np.mod(self.phi_init + (self.base_freq + f_offset) * t, 2 * np.pi)
...
# class SplineFootTrajectoryGenerator(FootTrajectoryGeneratorBase):
#     def compute_trajectory(self, t, f_offset, width, height):
#         phi = self.compute_phi(t, f_offset)  
...
# class SineFootTrajectoryGenerator(FootTrajectoryGeneratorBase):
#     def compute_trajectory(self, t, f_offset, width, height):
#         phi = self.compute_phi(t, f_offset)   
#         x = width / 2 * np.cos(phi)       
#         k = 2 * (phi - np.pi) / np.pi
...
# class BezierFootTrajectoryGenerator(FootTrajectoryGeneratorBase):
#     V_desire = 10
#     T_sw = 1
#     n = 11
#     P = np.array([
#         [-0.5, 0],
#         [-(0.5 + V_desire / ((n + 1) * T_sw) / 340), 0],
#         [-0.88, 0.73],
#         [-0.88, 0.73],
#         [-0.88, 0.73],
#         [0, 0.73],
...
#     def bezier_curve(self, P, t, width, height):
#         ret = np.array([[0., 0.]]).T * np.arange(len(t))
#         for i in range(self.n + 1):
#             ret += comb(self.n, i, exact=True) * P[i:i+1, :].T * np.power(1-t, self.n-i) * np.power(t, i)
#         ret[0] = width * ret[0]
#         ret[1] = height * ret[1]
#         return ret
...
#             self.bezier_curve(self.P, (phi - np.pi)/np.pi, width, height)
#         )
#         return ret
